...
inline data_t EncryptCFB(data_t data, sub_keys_t sub_keys)
{
    auto [s0, s1, s2, s3] = EncryptBlock({IV[0], IV[1], IV[2], IV[3]}, sub_keys);
    ...
    {
        std::tie(s0, s1, s2, s3) = EncryptBlock({ s0, s1, s2, s3 }, sub_keys);
...
inline data_t DecryptCFB(data_t data, sub_keys_t sub_keys)
{
    auto [s0, s1, s2, s3] = DecryptBlock({IV[0], IV[1], IV[2], IV[3]}, sub_keys);
